# 1 "/home/curs3w4ll/Documents/dev/MatriLED/Arduino/MatriLED/MatriLED.ino"
# 2 "/home/curs3w4ll/Documents/dev/MatriLED/Arduino/MatriLED/MatriLED.ino" 2
# 3 "/home/curs3w4ll/Documents/dev/MatriLED/Arduino/MatriLED/MatriLED.ino" 2
...
{
    for (short y = 0; y < 8; y++)
        Matrice.Content[Matrice_Number][y] = 0;
}
...
void Copy_Layout_In_Content(short Matrice_Number, byte Layout[8])
{
    for (short i = 0; i < 8; i++) {
        for (short y = 0; y < 8; y++) {
            (((((Layout[i]) >> (y)) & 0x01)) ? ((Matrice.Content[Matrice_Number][i]) |= (1UL << (y))) : ((Matrice.Content[Matrice_Number][i]) &= ~(1UL << (y))));
        }
    }
}
...
bool Write_Column(short Text_Global_Column, byte Layout[8], short Layout_Column)
{
    ...
    for (short Layout_Line = 0; Layout_Line < 8; Layout_Line++)
        (((((Layout[Layout_Line]) >> (Layout_Column)) & 0x01)) ? ((Matrice.Content[Matrice_Number][Layout_Line]) |= (1UL << (Matrice_Column_Position))) : ((Matrice.Content[Matrice_Number][Layout_Line]) &= ~(1UL << (Matrice_Column_Position))));
    return true;
}
